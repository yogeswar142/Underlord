...
# ── Interaction & Slash Error Handler ─────────────────────────────

async def on_tree_error(interaction: discord.Interaction, error: discord.app_commands.AppCommandError):
    """Global error handler for all slash commands."""
    await utils.report_error(interaction, error)

bot.tree.on_error = on_tree_error

# ── Universal UI Error Hooking ───────────────────────────────────
# We monkey-patch the base View and Modal classes so EVERY button and select
# in the entire bot automatically reports errors to your log channel.

from discord.ui import View, Modal

async def global_view_error(self, interaction: discord.Interaction, error: Exception, item):
    await utils.report_error(interaction, error)

async def global_modal_error(self, interaction: discord.Interaction, error: Exception):
    await utils.report_error(interaction, error)

View.on_error = global_view_error
Modal.on_error = global_modal_error
